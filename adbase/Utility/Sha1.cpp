...
Sha1::Sha1() {
	adbase::detail::reset(_digest, _buffer, _transforms);
}
...
void Sha1::update(std::istream &is) {
    while (true) {
        char sbuf[detail::BLOCK_BYTES];
        is.read(sbuf, detail::BLOCK_BYTES - _buffer.size());
        _buffer.append(sbuf, is.gcount());
        if (_buffer.size() != detail::BLOCK_BYTES) {
            return;
        }
        uint32_t block[detail::BLOCK_INTS];
		detail::buffer_to_block(_buffer, block);
		detail::transform(_digest, block, _transforms);
        _buffer.clear();
    }
}
...
std::string Sha1::final() {
    ...
    uint64_t total_bits = (_transforms*detail::BLOCK_BYTES + _buffer.size()) * 8;

    /* Padding */
    _buffer += 0x80;
    size_t orig_size = _buffer.size();
    while (_buffer.size() < detail::BLOCK_BYTES) {
        _buffer += (char)0x00;
    }
    ...
    if (orig_size > detail::BLOCK_BYTES - 8) {
		detail::transform(_digest, block, _transforms);
        for (size_t i = 0; i < detail::BLOCK_INTS - 2; i++) {
            block[i] = 0;
        }
    }

    /* Append total_bits, split this uint64_t into two uint32_t */
    block[detail::BLOCK_INTS - 1] = total_bits;
    block[detail::BLOCK_INTS - 2] = (total_bits >> 32);
	detail::transform(_digest, block, _transforms);
    ...
    /* Reset for next run */
	detail::reset(_digest, _buffer, _transforms);
...
void Sha1::final(char* result) {
    ...
    for (size_t i = 0; i < sizeof(_digest) / sizeof(_digest[0]); i++) {
		char tmp[4] = {0};
		tmp[0] = (_digest[i] >> 24) & 0xFF;
		tmp[1] = (_digest[i] >> 16) & 0xFF;
		tmp[2] = (_digest[i] >> 8) & 0xFF;
		tmp[3] = _digest[i] & 0xFF;
		memcpy(result + (i * sizeof(_digest[i])), tmp, sizeof(_digest[i]));
    }
...
void Sha1::hmacSha1(const char* key, size_t keylen, const char* msg, size_t msglen, char* out) {
	char ipad = 0x36;
	char opad = 0x5C;
	char paddedKey[64];
	char internalMsg[128]; // we will not manage big texts
	size_t t;
	char digest[20];
	...
	for (t = 0; t < 64; t++){
		...
		internalMsg[t] = paddedKey[t];
		internalMsg[t] ^= ipad;
	}

	for (t = 0; t < msglen; t++) {
		internalMsg[64 + t] = msg[t];
	}
	...
	Sha1 sha1;
	sha1.update(std::string(internalMsg, 64 + msglen));
	sha1.final(digest);
	...
	for (t = 0; t < 20; t++) {
		internalMsg[64 + t] = digest[t];
	}

	Sha1 finalSha1;
	finalSha1.update(std::string(internalMsg, 64 + 20));
	finalSha1.final(out);
}
