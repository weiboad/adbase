...
// }}}
// {{{ const std::string Request::getServerAddress()

const std::string Request::getServerAddress() const {
    struct evhttp_connection* conn = evhttp_request_get_connection(_request);
    struct bufferevent* buffer = evhttp_connection_get_bufferevent(conn);
    evutil_socket_t fd = bufferevent_getfd(buffer);
    if (fd == -1) {
        return "";
    }
    char ip[1024] = {0};
    adbase::sockets::toIpPort(ip, 1024, adbase::sockets::getLocalAddr(fd));
